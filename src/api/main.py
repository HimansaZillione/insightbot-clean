...
async def lifespan(app: fastapi.FastAPI):
    ...
    try:
        ...
        async with (
            ...
            if agent_id:
                try:
                    # If there's a colon, split into name/version, otherwise just use the ID
                    if ":" in agent_id:
                        ...
                        agent_version_obj = await project_client.agents.get(agent_id)
...
def create_app():
   ...
   # Explicitly load .env file from src/.env
    ...
    if os.path.exists(env_file):
        ...
        logger.warning(f".env file not found at {env_file}, relying on system environment variables")
